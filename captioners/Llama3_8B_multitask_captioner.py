...
def create_captions(
    ...
    def get_captions(INPs, adapter_name):
        all_caps = []
    ...
    for task in adapters:
        task_df = df[df["tasks"] == task]
        inps = [
            (smi, t)
            for smi, t in zip(task_df["SMILES"].tolist(), task_df["tasks"].tolist())
        ]
        captions = get_captions(inps, adapters[task])

        task_df["captions"] = captions
        dfs.append(task_df)
